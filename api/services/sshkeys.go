...
func (s *service) CreatePublicKey(ctx context.Context, key *models.PublicKey, tenant string) error {
	if err := key.Validate(); err != nil {
		fields, _ := validator.GetInvalidFields(err)

		return NewErrPublicKeyInvalid(fields, nil)
	}
	...
	if key.Filter.Tags != nil {
		if len(key.Filter.Tags) == 0 {
			return NewErrPublicKeyInvalid([]string{"tags"}, nil)
		}
	...
	if err != nil {
		return NewErrPublicKeyInvalid([]string{"data"}, nil)
	}
...
func (s *service) UpdatePublicKey(ctx context.Context, fingerprint, tenant string, key *models.PublicKeyUpdate) (*models.PublicKey, error) {
	if err := key.Validate(); err != nil {
		...
		return nil, NewErrPublicKeyInvalid(fields, nil)
	}
	...
	if key.Filter.Tags != nil {
		if len(key.Filter.Tags) == 0 {
			return nil, NewErrPublicKeyInvalid([]string{"tags"}, nil)
		}
